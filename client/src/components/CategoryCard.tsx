...
const CategoryCard: FC<CategoryCardProps> = ({ name, image }) => {
    ...
    return (
        <Link prefetch href={`/product?${createQueryString('category', name?.toString())}`} className='flex justify-center items-center w-full h-[150px]  md:h-[250px] lg:h-[300px] group hover:scale-110 transition-all duration-500 cursor-pointer rounded-xl hover:border-4 border-white hover:shadow-meraki'
            style={{
